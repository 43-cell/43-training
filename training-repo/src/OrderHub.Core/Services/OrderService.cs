...
public class OrderService : IOrderService
{
    ...
    public async Task<ServiceResult<Order>> CreateOrderAsync(int customerId, IReadOnlyList<NewOrderLine> lines)
    {
        ...
        var requestError = ValidateRequestShape(customer, lines);
        ...
        foreach (var line in lines)
        {
            ...
            var lineError = ValidateLineAndReserveStock(line, product, out var item);
    ...
    private static string? ValidateRequestShape(Customer? customer, IReadOnlyList<NewOrderLine> lines)
    {
        if (customer is null)
            return "找不到指定的客戶";
    ...
    private static string? ValidateLineAndReserveStock(NewOrderLine line, Product? product, out OrderItem? item)
    {
